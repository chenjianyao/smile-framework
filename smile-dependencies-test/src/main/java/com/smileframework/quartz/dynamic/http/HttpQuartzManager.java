 ...
 * 
 * @author chenjian
 * @since 2017年3月4日 下午4:55:37
 */
@Component
public class HttpQuartzManager {
	@Resource
	...
	public void removeJob(String jobName, String jobGroupName, String triggerName, String triggerGroupName) {
		try {
			...
			httpScheduler.pauseTrigger(triggerKey);// 停止触发器
			httpScheduler.unscheduleJob(triggerKey);// 移除触发器
			httpScheduler.deleteJob(JobKey.jobKey(jobName, jobGroupName));// 删除任务
		} catch (Exception e) {
			throw new RuntimeException(e);
		}
	}
	...
	public void modifyJobTime(String jobName, String jobGroupName, String triggerName, String triggerGroupName,
		...
		try {
			TriggerKey triggerKey = TriggerKey.triggerKey(triggerName, triggerGroupName);
			CronTrigger trigger = (CronTrigger) httpScheduler.getTrigger(triggerKey);
			if (trigger == null) {
				return;
			}
			...
			if (!oldTime.equalsIgnoreCase(cron)) {
				...
				// 方式一 ：修改一个任务的触发时间
				httpScheduler.rescheduleJob(triggerKey, trigger);
				/** 方式一 ：调用 rescheduleJob 结束 */

				/** 方式二：先删除，然后在创建一个新的Job */
				// JobDetail jobDetail =
				// httpScheduler.getJobDetail(JobKey.jobKey(jobName, jobGroupName));
	...
	public void addJob(String jobName, String jobGroupName, String triggerName, String triggerGroupName, Class jobClass,
		...
		try {
			...
			httpScheduler.scheduleJob(jobDetail, trigger);
